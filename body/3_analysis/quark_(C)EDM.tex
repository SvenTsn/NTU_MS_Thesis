...
As can be seen from \figref{fig:snowmass} taken from said report~\cite{Snow22}, progress on the nEDM front has stagnated for a decade or so,
...
We present the results for nEDM in \figref{fig:nEDM-fixed},
as well as combined results for eEDM and nEDM in the range \(r \in [0.6, 0.8] \) in \figref{fig:nEDM-eEDM},
with the ``extended'' ansatz~\eqnref{eq:ansatz} applied.
...
The interplay of the nEDM and the eEDM in \figref{fig:nEDM-eEDM} shows that our predictions are of notable significance in both precision observables.
The follow-up project at PSI, named n2EDM~\cite{n2EDM21}, plans to reach a sensitivity of \(\sim 10^{-27} \) \(e\,\mathrm{cm} \) within a decade, which covers the range illustrated in \figref{fig:nEDM-fixed}.

We have been utilizing the ``extended'' cancellation ansatz~\eqnref{eq:ansatz} in our above calculations and analyses, but we have to stress that,
...
We present our results in \figref{fig:nEDM-varied}.

\begin{figure}[t]
  \centering
  \includegraphics[width=4.9cm,height=3.7cm]{fig4_1.pdf}
  \includegraphics[width=4.3cm,height=3.7cm]{fig4_2.pdf}
  \includegraphics[width=5.26cm,height=3.7cm]{fig4_3.pdf}
...
This phenomenon in \figref{fig:nEDM-varied} illustrates a \textit{natural} cancellation mechanism present within the dynamics of nEDM,
