...
By ``squaring'' \eqnref{eq:DiracEqwithEMField} one obtains
\begin{align}
...
In {\gthdm}, the first finite contribution to EDM appears at one-loop (\figref{fig:oneloop}).
...
A certain set of two-loop diagrams, known as Barr-Zee diagrams (\figref{fig:BarrZee_general}), are indeed of importance.
...
We also explicitly draw out all of the corresponding diagrams in~\figref{fig:BarrZee-specific}.
...
\end{equation}
which correspond to the diagrams in~\figref{fig:chromo-edm-all}.
